...
var DefaultToolPrompts = map[string]string{
	"hangup":         "* Ferramenta hangup (Desligar Chamada): Use esta ferramenta APENAS E EXCLUSIVAMENTE quando o cliente disser explicitamente que não precisa de mais nada, se despedir ou confirmar que o atendimento está encerrado. REGRA MANDATÓRIA: Antes de chamar esta ferramenta, você DEVE SEMPRE falar em voz alta uma despedida calorosa e educada para o cliente (ex: \"Muito obrigado pelo contato, foi um prazer te atender! Tenha um ótimo dia, até logo!\"). NUNCA chame a ferramenta hangup silenciosamente ou sem falar a despedida primeiro.",
